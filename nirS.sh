...
cutadapt -g "AACGYSAAGGARACSGG;min_overlap=15" -G "GASTTCGGRTGSGTCTTSAYGAA;min_overlap=21" -j 0 --discard-untrimmed -o /media/dave/storage/macronutrient/BFI_paper/data/sequences/nirS/${f%L001_R1.fastq.gz}R1_primer_trimmed.fastq -p /media/dave/storage/macronutrient/BFI_paper/data/sequences/nirS/${f%L001_R1.fastq.gz}R2_primer_trimmed.fastq $f ${f%R1.fastq.gz}R2.fastq.gz
done
...
R
library(data.table)
dat <- fread("grep '^[^>]' FEB-CE1_3_b_AGGCAGAA-ACTGCATA.fna | awk '{print length}'")#"
summary(dat$V1)
q()

# take all reads between 360-390
for f in *.fna; do
awk '!/^>/ { next } { getline seq } length(seq) >= 360 { print $0 "\n" seq }' $f | awk '!/^>/ { next } { getline seq } length(seq) <= 390 { print $0 "\n" seq }' > ${f%.fna}_len_filtered.fna
done
...
paste(dat[V2 <= 3900, V1], collapse = ",")
q()

rm -f {AUG-AS3_1_b_TCCTGAGC-AAGGAGTA_len_filtered.fna,FEB-GA2_1_b_CGAGGCTG-GTAAGGAG_len_filtered.fna,FEB-GA2_2_b_AAGAGGCA-GTAAGGAG_len_filtered.fna}

# concatenate all length filtered fasta files into one
cat *_len_filtered.fna > allSequences.fna

# get total final lib sizes
grep -c "^>" allSequences.fna # 705516
...
java -Xmx16g -jar ~/bioinformatics/RDPTools/FrameBot.jar framebot -o framebotOut/nirS -N -i 0.5 -l 50 /media/dave/storage/PRINCE/data/sequenceData/fungeneDBs/nirS/fungene_nirS.fa uniq_DNA_seqs.fna
...
~/bioinformatics/usearch11.0.667_i86linux32 -otutab uc_nirS_corr_prot.fasta -otus AAVs.fasta -id 1 -minsize 0 -otutabout nirS_AAV.txt -qmask none -dbmask none

# use vsearch to dereplicate frameshift corrected nucleotide sequences
vsearch --derep_fulllength nirS_corr_nucl.fasta --output derep_nucl_seqs.fna --minuniquesize 2 --relabel OTU --fasta_width 0 --sizein --sizeout

# sort by cluster size
vsearch --sortbysize derep_nucl_seqs.fna --output derepSorted.fna

# pick otu centroids
vsearch --cluster_size derepSorted.fna --id 0.97 --centroids nirSCentroids.fna

# create map reads to OTUs
vsearch --usearch_global nirS_corr_nucl.fasta --db nirSCentroids.fna --id 0.97 --log nirSOtus.log --otutabout nirSOtuTab.txt --sizein
